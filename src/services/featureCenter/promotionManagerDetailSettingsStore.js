...
const ENTRY_ID = 'promotion-master-new-campaign-detail';
const CONFIG_FILE_NAME = 'detail-settings.json';

function createPromotionManagerDetailSettingsStore({
  sessionStore,
  ...
  const base = createStoreBase({
    ...
    featureCenterProfileService,
    entryId: ENTRY_ID,
    entryNotRegisteredMessage: '\u63a8\u5e7f\u5927\u5e08\u63a8\u5e7f\u660e\u7ec6\u6a21\u5757\u672a\u6ce8\u518c\uff0c\u65e0\u6cd5\u8bfb\u5199\u914d\u7f6e\u3002'
  });
  ...
  async function readCloudConfig(owner) {
    const key = getCloudConfigKey(owner);
  ...
  async function writeUserConfig(payload, metadata = {}) {
    ...
    try {
      await cosService.putJson({
        ...
        data: payload,
        metadata: {
          record_type: 'promotion-manager-detail-settings',
