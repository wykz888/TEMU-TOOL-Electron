...
const ENTRY_ID = 'operations-activity-management';
const LEGACY_STORAGE_KEYS = Object.freeze([
  'feature_center/operations_management/activity_management'
]);
const CONFIG_FILE_NAMES = Object.freeze({
  filterSettings: 'filter-settings.json',
  productFilterMode: 'product-filter-mode.json',
  productFilterModeValues: 'product-filter-mode-values.json',
  productFilterProfitFloorRate: 'product-filter-profit-floor-rate.json'
});

function createOperationsActivityManagementStore({
  sessionStore,
  ...
  const base = createStoreBase({
    ...
    featureCenterProfileService,
    entryId: ENTRY_ID,
    entryNotRegisteredMessage: '\u6d3b\u52a8\u7ba1\u7406\u6a21\u5757\u672a\u6ce8\u518c'
  });
  ...
  async function writeUserConfig(configKey, payload, metadata = {}) {
    ...
    try {
      await cosService.putJson({
        ...
        data: payload,
        metadata: {
          record_type: `operations-activity-management-${configKey}`,
