...
} from '../imageUpload/useImageUploadDialog.js';

const DEFAULT_VIEW_BRIDGE_KEY = 'podUploadSheetMiaoshouViewBridge';
...
function normalizeOutputLanguage(value) {
  return value === 'zh' ? 'zh' : DEFAULT_OUTPUT_LANGUAGE;
}
...
function getViewBridge(viewBridgeKey) {
  return window[viewBridgeKey] && typeof window[viewBridgeKey] === 'object'
    ? window[viewBridgeKey]
    : null;
}
...
function createDefaultFormState(includeOutputLanguage) {
  const formState = {
  ...
  if (includeOutputLanguage) {
    formState.outputLanguage = DEFAULT_OUTPUT_LANGUAGE;
  }
...
export function createBatchAiTitleDialog(options = {}) {
  const includeOutputLanguage = options.includeOutputLanguage === true;
  const viewBridgeKey = normalizeText(options.viewBridgeKey) || DEFAULT_VIEW_BRIDGE_KEY;
  const visible = ref(false);
  const loading = ref(false);
  const starting = ref(false);
  const form = reactive(createDefaultFormState(includeOutputLanguage));
  ...
  function collectPayload(retryFailedOnly = false) {
    ...
    if (includeOutputLanguage) {
      payload.outputLanguage = normalizeOutputLanguage(form.outputLanguage);
    }
  ...
  async function startGeneration(retryFailedOnly = false) {
    const bridge = getViewBridge(viewBridgeKey);
  ...
  function installGlobalBridge() {
    const existingBridge = getViewBridge(viewBridgeKey) || {};
    const bridge = {
      ...existingBridge,
    ...
    window[viewBridgeKey] = bridge;

    return function cleanupGlobalBridge() {
      if (window[viewBridgeKey] !== bridge) {
        return;
      }
      ...
      if (Object.keys(nextBridge).length) {
        window[viewBridgeKey] = nextBridge;
        return;
      }

      delete window[viewBridgeKey];
    };
  }
  ...
  return {
    ...
    outputLanguageOptions: includeOutputLanguage ? OUTPUT_LANGUAGE_OPTIONS : [],
