...
- Promotion manager new goods query: `promotionManagerNewGoodsService.js` owns `query_mall_goods_list` payload normalization, multi-shop/multi-region fetch orchestration, task cancellation, and goods row parsing for the Vue renderer. Goods list pages are capped at 100 rows per backend request, and `promotionManagerNewBidUtils.js` caps `getBid` prediction chunks at 50 goods per request. Promotion manager new ad creation is isolated in `promotionManagerNewAdsCreateService.js`, uses the same region-cookie backend session path, submits `create_ads/create` in capped chunks with row-level create status results, and returns only the clone-safe result model from `promotionManagerNewAdsCreateResult.js` across IPC.
- Promotion manager new create settings: `promotionManagerNewSettingsService.js` and `promotionManagerNewSettingsStore.js` persist the new create-promotion selection settings under the `promotion-master-new-campaign-create` storage boundary, with local config plus COS sync independent of the old promotion manager settings service. The renderer-side selection load/save adapter lives in `src/renderer/promotionManagerNewApp/services/createPromotionSettings.js`. Promotion manager new monitor shop-list loading is kept in the renderer service `src/renderer/promotionManagerNewApp/services/promotionMonitorShops.js`, with row normalization in `src/renderer/promotionManagerNewApp/view-models/promotionMonitorShopRows.js`.
