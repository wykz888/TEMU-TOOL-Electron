...
- Promotion manager goods query: `promotionManagerNewGoodsService.js` owns `query_mall_goods_list` payload normalization, multi-shop/multi-region fetch orchestration, task cancellation, and goods row parsing for the Vue renderer. Goods list pages are capped at 100 rows per backend request, and `promotionManagerNewBidUtils.js` caps `getBid` prediction chunks at 50 goods per request. Promotion manager ad creation is isolated in `promotionManagerNewAdsCreateService.js`, uses the same region-cookie backend session path, submits `create_ads/create` in capped chunks with row-level create status results, and returns only the clone-safe result model from `promotionManagerNewAdsCreateResult.js` across IPC.
