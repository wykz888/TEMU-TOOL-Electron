...
- Current split: `shopWindowViewLoadErrorPage.js` owns browser load failure text/data-URL rendering, `shopWindowPartitionIdentity.js` owns shop partition identity hashes and partition directory resolution, `shopWindowContextMenu.js` owns native right-click browser menu construction for shop WebContentsView pages, and `shopWindowEnvironmentSignature.js` owns proxy/fingerprint environment comparison for workspace reuse and runtime-profile refresh decisions. Shop detail proxy merge rules live in `src/services/shopManagement/shopDetailProxyConfig.js` so saved local/direct proxy settings cannot be overwritten by stale shop-index summaries. Keep future pure browser-controller rules in focused helpers before adding to `shopWindowBrowserController.js`.
...
- Promotion Ads session boundary: `promotionAdsSessionService.js` owns TEMU ads login readiness, region cookie refresh/cache, and backend region-cookie POST requests. Its cookie cache is stored under the current `promotion-master-new` storage boundary to preserve existing user data, but the visible product entry is now the official Promotion Master. `promotionMasterSessionService.js` remains only as a compatibility export; newer code should depend on the neutral Ads session service naming.
- Promotion manager goods query: `promotionManagerNewGoodsService.js` owns `query_mall_goods_list` payload normalization, multi-shop/multi-region fetch orchestration, task cancellation, and goods row parsing for the Vue renderer. Goods list pages are capped at 100 rows per backend request, and `promotionManagerNewBidUtils.js` caps `getBid` prediction chunks at 50 goods per request. Promotion detail query is isolated in `promotionManagerNewDetailService.js`; `promotionManagerNewDetailRows.js` builds time-bounded `ads_detail` payloads and clone-safe row/result models. Detail queries use the same region-cookie backend session path, page with cancellation support, and feed the detail table through IPC/preload. Promotion detail manual operations are isolated in `promotionManagerNewDetailActionService.js`; `promotionManagerNewDetailActionRows.js` owns row normalization/grouping and `modify_ad_dtos` shaping, `promotionManagerNewDetailActionApi.js` owns response parsing, and `promotionManagerNewDetailActionResult.js` owns clone-safe result sanitization. The action service submits in chunks over the region-cookie backend session path and supports cancellation. Promotion manager ad creation is isolated in `promotionManagerNewAdsCreateService.js`, uses the same region-cookie backend session path, submits `create_ads/create` in capped chunks with row-level create status results, and returns only the clone-safe result model from `promotionManagerNewAdsCreateResult.js` across IPC.
- Promotion manager create settings: `promotionManagerCreateSettingsModel.js`, `promotionManagerNewSettingsService.js`, and `promotionManagerNewSettingsStore.js` persist create-promotion selection, goods filter drafts/applied filters, and batch setting defaults under the `promotion-master-new-campaign-create` storage boundary as `create-promotion-settings.json`, with local config plus COS sync independent of the removed old promotion manager entry. The renderer-side load/save adapter lives in `src/renderer/promotionManagerNewApp/services/createPromotionSettings.js`. Promotion manager detail settings use `promotionManagerDetailSettingsModel.js`, `promotionManagerDetailSettingsStore.js`, and `promotionManagerDetailSettingsService.js` to sync detail-page shop/region/date/filter/action preferences under the `promotion-master-new-campaign-detail` storage boundary as `detail-settings.json`; the renderer adapter is `src/renderer/promotionManagerNewApp/services/promotionDetailSettings.js`. Promotion manager monitor settings use the `promotion-master-new-campaign-monitor` storage boundary through a parameterized settings service plus dedicated preload/IPC methods. Promotion monitor config and action model rules live in `promotionMonitorConfigModel.js`, metrics parsing lives in `promotionMonitorMetricUtils.js`, `ads_detail` item normalization lives in `promotionMonitorAdsDetailItemParser.js`, `ads_detail` paging and backend region-cookie collection lives in `promotionMonitorAdsDetailItems.js`, automatic pause predicates plus spend-sort and empty-region throttle rules live in `promotionMonitorOperationRules.js`, pause/resume sequence decisions live in `promotionMonitorPauseSequenceRules.js`, modify_ads request shaping lives in `promotionMonitorModifyPayloads.js`, runtime state/default/persistable payload rules live in `promotionMonitorStateModel.js`, runtime logs live in `promotionMonitorRuntimeLogStore.js`, and `promotionMonitorService.js` remains the main-process monitor orchestration layer. Promotion manager monitor shop-list loading is kept in the renderer service `src/renderer/promotionManagerNewApp/services/promotionMonitorShops.js`, with row normalization in `src/renderer/promotionManagerNewApp/view-models/promotionMonitorShopRows.js`.
