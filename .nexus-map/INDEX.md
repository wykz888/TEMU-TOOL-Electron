...
- Shop browser automation is a major main-process side system under `src/windows/`, especially `shopWindowBrowserController.js`, login autofill, seller session probes, and seller-center landing logic. Pure browser load-error page rendering now lives in `src/windows/shopWindowViewLoadErrorPage.js`, shop browser partition identity/path rules live in `src/windows/shopWindowPartitionIdentity.js`, and browser right-click menu behavior lives in `src/windows/shopWindowContextMenu.js` so the controller stays focused on WebContentsView orchestration.
